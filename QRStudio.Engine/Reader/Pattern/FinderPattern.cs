...
namespace QRStudio.Engine.Codec.Reader.Pattern
{
    ...
    public class FinderPattern
	{
		...
		internal static Point[] getCenter(Line[] crossLines)
		{
			...
			{
				canvas.drawPolygon(foundPoints, QRStudio.Engine.Codec.Util.Color_Fields.RED);
				return foundPoints;
			}
			else
		...
		internal static int calcExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
		{
			...
			Point target;
			Axis axis = new Axis(angle, moduleSize[UR]); 
			...
			canvas.drawPoints(points, QRStudio.Engine.Codec.Util.Color_Fields.RED);
